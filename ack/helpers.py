...
from .env import OPENAI_ORGANIZATION_ID, OPENAI_KEY
...
def get_oai_model() -> str:
    if datetime.date.today().year >= 2024:
...
def run_evaluation(prompt: str):
    return chat_completion(
        prompt, hacker_news.oai_evaluation_role_instructions
    )


def run_reword(prompt: str):
    return chat_completion(prompt, hacker_news.oai_reword_role_instructions)
